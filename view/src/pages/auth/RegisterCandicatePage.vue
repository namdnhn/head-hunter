					...
					<form class="space-y-4 md:space-y-6" action="#">
						<div>
							...
							<input type="email" name="email" id="email"
								...
								placeholder="email@company.com" required="true" v-model="registerEmailCandicate.value" />
						</div>
						<div>
							...
							<input type="password" name="password" id="password" placeholder="••••••••"
								...
								required="true" v-model="registerPasswordCandicate.value" @input="checkKeyDownPassword" />
						</div>
						<div>
							<!-- <label v-if="checkKeyDown()"></label> -->
							<label class="block mb-2 text-sm font-medium text-red-500"
								v-if="!registerPasswordCandicate.isValid">Mật khẩu của bạn quá ngắn, hãy nhập lại mật
								khẩu !</label>
						</div>
						<div>
							<label for="password" class="block mb-2 text-sm font-medium text-cyan-900">Xác nhận mật
								khẩu</label>
							<input type="password" name="password" id="password" placeholder="••••••••"
								class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
								required="true" v-model="registerPasswordCandicateConfirm.value"
								@input="checkKeyDownPasswordConfirm" />
						</div>
						<!-- fullname  -->
						<div>
							<label for="fullname" class="block mb-2 text-sm font-medium text-cyan-900">Họ và tên</label>

							<input type="text" name="fullname" id="fullname"
								class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
								required="true" v-model="fullnameCandidate.value" />
						</div>
						<!-- date of birth  -->
						<div>
							<label for="dob" class="block mb-2 text-sm font-medium text-cyan-900">Ngày sinh</label>

							<input type="date" name="dob" id="dob"
								class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
								required="true" v-model="dobCandidate.value" />
						</div>
						<!-- phone number  -->
						<div>
							<label for="phone" class="block mb-2 text-sm font-medium text-cyan-900">Số điện thoại</label>

							<input type="text" name="phone" id="phone"
								class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
								required="true" v-model="phoneCandidate.value" />
						</div>
						<div>
							<!-- <label v-if="checkKeyDown()"></label> -->
							<label class="block mb-2 text-sm font-medium text-red-500"
								v-if="!registerPasswordCandicateConfirm.isValid">Mật khẩu không khớp với mật khẩu đăng
								ký</label>
						</div>
							...
							<div class="flex items-start">
								<div class="flex items-start">
									...
									<div class="ml-3 text-sm">
										<label for="terms" class="font-light text-cyan-900">Tôi đồng ý với
											<a class="font-medium text-primary-600 hover:underline dark:text-primary-500"
												href="#" required="true">Điều khoản và điều kiện của
												Head Hunter</a></label>
									</div>
								</div>
							</div>
						...
						<p class="text-sm font-light text-cyan-800">
							Bạn đã có tài khoản ?
							<a href="#" class="font-bold text-primary-600 hover:underline dark:text-primary-500">
								<router-link to="./loginCandidatePage">Đăng nhập ngay !</router-link>
							</a>
						</p>
					</form>
				</div>
...
export default {
	...
	methods: {
		validateForm() {
			...
			if (this.registerPasswordCandicate.value === '' || this.registerPasswordCandicate.value.length < 8) {
				this.registerPasswordCandicate.isValid = false;
				this.formIsValid = false;
			}
			if (this.registerPasswordCandicateConfirm.value === '' || this.registerPasswordCandicateConfirm.value.length < 8 || this.registerPasswordCandicateConfirm.value !== this.registerPasswordCandicate.value) {
